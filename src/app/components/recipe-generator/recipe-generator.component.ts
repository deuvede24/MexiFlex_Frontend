...
// === MODIFICACIÓN: Actualizada interface para soportar función en ingredients ===
...
export class RecipeGeneratorComponent implements OnInit, OnDestroy {
  ...
  // === MODIFICACIÓN: Añadido sistema de cache ===
  ...
  // === MODIFICACIÓN: Actualizado ingredientCategories con función para carbohidratos ===
  ...
  // === MODIFICACIÓN: Actualizado ngOnInit y ngOnDestroy para manejar cache ===
  ngOnInit(): void {
    this.cacheCleanupInterval = setInterval(() => this.cleanOldCache(), 1000 * 60 * 5);
  }

  // === MODIFICACIÓN: Añadida función para filtrar carbohidratos ===
  private getAvailableCarbsForRecipeType(recipeType: string): string[] {
    switch(recipeType) {
      ...
      case 'ensalada':
        return ['Tostadas', 'Pan Bolillo'];
      case 'sopa':
        return ['Arroz', 'Pan Bolillo', 'Pasta'];
      default:
  ...
  // Todo el manejo de ingredientes se mantiene igual
